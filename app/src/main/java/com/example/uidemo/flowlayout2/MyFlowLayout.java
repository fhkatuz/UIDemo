...
import com.example.uidemo.utils.DPUtils;
...
public class MyFlowLayout extends ViewGroup {
    ...
    public MyFlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        DEFAULT_HORIZONTAL_MARGIN = DPUtils.dip2px(context, 5f);//元素之间的水平间距
        DEFAULT_VERTICAL_MARGIN = DPUtils.dip2px(context, 5f);//元素之间的竖直间距
        DEFAULT_TEXT_MAX_LENGTH = -1;//元素里面的字体长度限制
        DEFAULT_BORDER_RADIUS = DPUtils.dip2px(context, 5f);//元素外面的边框弧度
        //获取属性
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (itemWith>0&&itemHeight>0){
                FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(DPUtils.dip2px(mContext,itemWith),DPUtils.dip2px(mContext,itemHeight));
